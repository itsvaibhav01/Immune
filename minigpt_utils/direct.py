...
class TQ_direct:
    ...
    @timeit
    def generate(self, prompt, redteam_query, stopping_criteria, weight=0.5, topk=2, max_new_token=64, method="topp", temperature=0.7, chunk_size=5, max_new_tokens=5, prompt_forward=False, debug=True):
        input_embeds = prompt.context_embs[0]
        tokens = self.get_input_ids(prompt.text_prompts, add_special_tokens=True)
        if debug: logging.info(f"First input tokens: {tokens}")

        initial_len = input_embeds.shape[1]
        if chunk_size == "auto":
        ...
        self.redteam_query = "<|user|>: " + redteam_query + ' <|assistant|>: '
        ...
        for _ in iterator_obj:
            ...
            with torch.no_grad():
                if cached is None:
                    mout = self.LLM(inputs_embeds=input_embeds, past_key_values=None, use_cache=True, return_dict=True)
                    cached = mout.past_key_values
                else:
                    mout = self.LLM(new_token, past_key_values=cached, use_cache=True, return_dict=True)
                    cached = mout.past_key_values

                tokens, scores, rm_cached = self.generate_step(mout, tokens, prompt_token_len, topk, max_new_tokens, weight, method, temperature, prompt_forward, debug, scores, rm_cached)

                # logging.info(f'tokens.shape: {tokens.shape}')
                new_token = tokens[:, -1:]
                # logging.info(f"Selected new token: {new_token}")
                del mout
    
            # stop gen on stop token
            if stopping_criteria(tokens, scores): break
