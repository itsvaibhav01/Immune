...
import random
from minigpt_utils import prompt_wrapper, generator_attack
...
class Attacker:
    ...
    def attack_unconstrained(self, text_prompt, img, batch_size = 8, num_iter=2000, alpha=1/255):
        ...
        my_generator = generator_attack.Generator(model=self.model, device=self.model.device)
        ...
        for t in tqdm(range(num_iter + 1)):
            ...
            prompt = prompt_wrapper.Prompt(model=self.model, text_prompts=text_prompts, img_prompts=[[x_adv.to(self.device)]], device=self.device)
    ...
    def attack_constrained(self, text_prompt, img, batch_size = 8, num_iter=2000, alpha=1/255, epsilon = 128/255 ):
        ...
        my_generator = generator_attack.Generator(model=self.model, device=self.device)
